...
// RUN: %webcl-validator %s -- -I%include -include _kernel.h -include 1st-level.h 2>/dev/null | grep -v "Processing\|CHECK\|DRIVER-MAY-REJECT" | %opencl-validator
// RUN: %webcl-validator %s -- -I%include -include _kernel.h -include 1st-level.h 2>&1 | grep -v CHECK | %FileCheck %s
